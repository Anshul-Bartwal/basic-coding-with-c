...
function createTestCasesElement(testCases) {
  ...
  testCases.forEach(({ input, output, explanation }, idx) => {
    const container = document.createElement("div");// Load questions JSON (replace path if necessary)
async function loadQuestionsData() {
  const response = await fetch("questions2.json");
  return response.json();
}
...
function createQuestionElement(q, dayNum, currentDay) {
  ...
  function formatCopyText(question) {
    // Comment the question title and description
    let questionPart = `Q${question.id}: ${question.text}`;
    const questionLines = questionPart.split('\n');
    const commentedQuestion = questionLines.map(line => {
    ...
    // Multi-line comment for test cases (as before)
    let testCasesPart = '/*\nSample Test Cases:\n';
    question.testCases.forEach((tc, idx) => {
      testCasesPart += `Input ${idx + 1}:\n${tc.input}\nOutput ${idx + 1}:\n${tc.output}\n\n`;
    });
    testCasesPart += '*/';

    return commentedQuestion + testCasesPart;
  }
